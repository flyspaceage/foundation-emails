...
gulp.task('build',
  gulp.series(clean, pages, sass, images, inline, nano));
...
function nano() {
  return sass('src/assets/scss/app.scss', { style: 'expanded' })
        .pipe(cssnano())
        .pipe(gulp.dest('dist/css'));
}
...
function watch() {
  ...
  gulp.watch(['../scss/**/*.scss', 'src/assets/scss/**/*.scss']).on('all', gulp.series(resetPages, sass, pages, inline, nano, browser.reload));
